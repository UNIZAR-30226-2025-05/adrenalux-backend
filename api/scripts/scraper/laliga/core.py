...
class LaLigaScraper:
    ...
    def to_json(self):
        logger.info(f'Dumping player dataframe to {self.seasoned_output_filepath.stem}.json')
        # Convertir el DataFrame a un formato de diccionario y guardarlo en JSON
        self.df.to_json(self.seasoned_output_filepath.with_stem(f'S{self.season}-{self.output_filepath.stem}').with_suffix('.json'), orient='records', lines=True)